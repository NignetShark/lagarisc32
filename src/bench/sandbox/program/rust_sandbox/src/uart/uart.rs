...
static mut UART_INST: Uart = Uart { base_address : 0x1000_0000 };
...
impl Uart {
    ...
    pub fn init(&self) {
        let uart_reg = self.base_address as *mut u8;
        ...
        unsafe {
            ...
            //             ~~~~~~     ~~~~~~
            let lcr: u8 = (1 << 0) | (1 << 1);
            uart_reg.add(3).write_volatile(lcr);
            ...
            uart_reg.add(3).write_volatile(lcr | 1 << 7);

            // Now, base addresses 0 and 1 point to DLL and DLM,
            // respectively. Put the lower 8 bits of the divisor
            // into DLL
            uart_reg.add(0).write_volatile(divisor_least);
            uart_reg.add(1).write_volatile(divisor_most);
    ...
    pub fn put(&self, c: u8) {
        let uart_reg = self.base_address as *mut u8;
        unsafe { uart_reg.add(0).write_volatile(c) };
    }

    pub fn get(&self) -> Option<u8> {
        let uart_reg = self.base_address as *mut u8;
        unsafe {
            if uart_reg.add(5).read_volatile() & 1 == 0 {
                // The DR bit is 0, meaning no data
                None
            } else {
                // The DR bit is 1, meaning data!
                Some(uart_reg.add(0).read_volatile())
            }
        }
    }
    ...
    pub fn global_init() {
        ...
        unsafe {
            UART_INST = Uart::new(0x1000_0000);
        }
    }

    #[allow(static_mut_refs)]
    pub fn get_global() -> &'static mut Uart {
        unsafe {
            &mut UART_INST
        }
    }
